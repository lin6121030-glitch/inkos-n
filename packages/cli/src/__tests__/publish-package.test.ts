...
describe.sequential("publish packaging", () => {
  it("rewrites workspace package versions for canary publishing", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "inkos-n-core", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
      ...
      expect(cliPackageJson.dependencies["inkos-n-core"]).toBe("0.4.8-canary.7");
    } finally {
  ...
  it("keeps source CLI dependencies linked through the workspace protocol", async () => {
    ...
    expect(cliPackageJson.dependencies["inkos-n-core"]).toBe("workspace:*");
    expect(cliPackageJson.dependencies["inkos-n-studio"]).toBe("workspace:*");
  });
  ...
  it("allows source workspace protocol manifests when they normalize cleanly for publish", async () => {
    ...
    try {
      ...
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "inkos-n-core", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
  ...
  it("replaces workspace dependencies before npm pack", { timeout: 30_000 }, async () => {
    ...
    try {
      ...
      expect(packedPackageJson.dependencies["inkos-n-core"]).toBe(corePackageJson.version);
      expect(packedPackageJson.dependencies["inkos-n-studio"]).toBe(studioPackageJson.version);
    } finally {
