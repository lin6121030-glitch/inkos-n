...
export const updateCommand = new Command("update")
  ...
  .action(async () => {
    try {
      ...
      const remoteVersion = execSync("npm view inkos-n version", {
        encoding: "utf-8",
      }).trim();
      ...
      execSync("npm install -g inkos-n@latest", { stdio: "inherit" });
      log(`Updated to ${remoteVersion}.`);
    } catch (e) {
      logError(`Update failed: ${e}`);
      log("You can also update manually: npm install -g inkos-n@latest");
      process.exit(1);
    }
  });
